...
<head>
    ...
    <style>
        body {
            ...
            color: #333;
            background-image: url('https://i.imgur.com/tTQSAYw.jpg');
        ...
        /* Agregar la transformación para invertir la imagen del Digimon enemigo */
        .digimon img.enemigo {
            transform: scaleX(-1); 
        }
...
<body>
    ...
    <div class="container">
<h1 class="{{ $colorClase }}">{{ $nombreRuta }}</h1>
...
@endif
        ...
        @if(!session('redireccionar'))
        <form action="{{ route('accion.combate2') }}" method="POST">
    @csrf
...
@php
        $cancion = \App\Models\ListaCancion::where('nombre', 'CombOnline')->first();
    @endphp
